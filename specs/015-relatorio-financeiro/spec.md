...
Este é o único dos relatórios "de listagem" (Estórias 12-14, 16-18) que o endpoint original (`GET /api/relatorios/financeiro`) é efetivamente consumido por uma tela. Além disso, o sistema evoluiu bem além do que a ERS previa: existe hoje uma tela "Financeiro → Visão Geral" separada (não baseada em `/api/relatorios`, mas em um endpoint próprio combinando Contas a Receber e a Pagar) e uma segunda aba de "Visão de Indicadores" (inadimplência, margem de segurança, fluxo de caixa) adicionada em sprint posterior, sem equivalente na ERS original.
...
Como professora, eu acesso a aba "Visão de Indicadores" do Dashboard Financeiro para ver Inadimplência %, Prazo médio de atraso, Margem de segurança %, Cobertura de custos e um painel de "Gargalo de caixa", além do fluxo de caixa dos últimos 6 meses.
...
1. **Given** a aba Visão de Indicadores, **When** carregada, **Then** exibe Inadimplência %, Prazo médio de atraso, Margem de segurança %, Cobertura de custos (recebido ÷ pago) e o dia de maior entrada/saída ("Gargalo de caixa").
...
- **FR-002**: O sistema MUST agrupar os valores recebidos por Forma de Pagamento, por Aluno e por Turma nessa mesma consulta.
...
- **FR-005**: O sistema MUST calcular indicadores financeiros avançados (inadimplência, prazo médio de atraso, margem de segurança, cobertura de custos, dia de maior entrada/saída, fluxo de caixa dos últimos 6 meses), filtráveis por período, turma, matéria e aluno.
